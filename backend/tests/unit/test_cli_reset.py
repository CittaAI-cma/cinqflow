...
def reset_env(tmp_path, monkeypatch, settings):
    """A throwaway set of the four schemas `reset` owns, isolated from every
    other test the same way conftest's own fixtures are (random suffix)."""
    import uuid
    ...
    with psycopg.connect(s.database_url) as conn:
        with conn.cursor() as cur:
            for schema in (s.workflow_schema, s.queue_schema, s.silver_schema, Layer.BRONZE.value):
